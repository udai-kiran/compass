...
## Status
APPROVED FOR PR-A ONLY (dual-write iteration 3 — review-7 verdict APPROVED-FOR-PR-A; both PR-A-gating blockers confirmed RESOLVED against real code). PR-A is the first buildable, green, RELEASABLE increment. **PR-G staging is NOT yet finalized** and re-enters PLAN_REVIEW before PR-G is built: review-7 leaves blocker 3 PARTIAL (line-17/G2 date now reconciled here) and blocker 4 NOT-RESOLVED (G1–G4 are not yet independently deployable — a G1 legacy dual-writer cannot represent a G2-collapsed transfer; a compatibility/flip boundary or an explicit indivisible G2→G4 maintenance deployment must be specified, plus a distinct post-collapse invariant for G3).
...
- Restore compatibility (blocker 4 / D19): fresh-account guard ignores `system_kind IS NOT NULL`; seeded system accounts retained/regenerated (not deleted by the reverse-order loop); build old→new system-account id map by `system_kind`; rewrite each restored posting's `account_id`; never insert archived system accounts as ordinary; JSON archive round-trips postings. **OLD archives (legacy transactions but NO postings): restore must SYNTHESIZE postings from the restored legacy transactions/splits/is_opening/transfer_links BEFORE commit (else the per-transaction invariant is violated on restore); NEWER archives' posting shapes are VALIDATED (rebuild-and-compare), not blindly trusted.** (CSV projection can wait to PR-F; JSON backup/restore correctness cannot.)
