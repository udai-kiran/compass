...
async function accessTokenFor(
  mb: MailboxRow,
): Promise<{ token: string; host: string; port: number }> {
...
async function syncPass(mb: MailboxRow): Promise<void> {
  ...
  try {
    ...
    if (plan.baseline) {
      ...
      log("info", "mailbox baselined", {
        mailbox: mb.emailAddress,
        lastUid: plan.baseline.lastUid,
      });
      return;
    }
    ...
    const newLastUid = advanceLastUid(
      mb.lastUid!,
      fresh.map((m) => m.uid),
    );
...
async function runAll(userId?: string): Promise<void> {
  const mailboxes = await loadSyncableMailboxes(pool, userId).catch((err: unknown) => {
...
/**
 * Run a sync pass, debounced: if one is already in flight (from the timer or an
 * on-demand trigger), just flag a re-run so nothing overlaps and no request is lost.
 */
async function trigger(userId?: string): Promise<void> {
  if (stopped) return;
  if (running) {
    if (userId === undefined) pendingAll = true;
    else pendingUsers.add(userId);
    return;
  }
  running = true;
  try {
    ...
    running = false;
    if (!stopped && pendingAll) {
      pendingAll = false;
      pendingUsers.clear();
      void trigger();
    } else if (!stopped && pendingUsers.size > 0) {
      const nextUserId = pendingUsers.values().next().value as string;
      pendingUsers.delete(nextUserId);
      void trigger(nextUserId);
    }
  }
}
...
// A user queued a sync from the app. The API enqueues onto INGESTOR_QUEUE with a
// delay (the chosen window) and coalesces requests, so by the time the job fires
// here it's a single pending run — execute one pass.
const runWorker = new Worker(
  INGESTOR_QUEUE,
  async (job) => {
    log("info", "running queued sync");
    const parsed = z.object({ userId: z.uuid() }).parse(job.data);
    await trigger(parsed.userId);
  },
  { connection: { url: config.REDIS_URL, maxRetriesPerRequest: null } },
);
runWorker.on("error", (err) => log("error", "ingestor worker error", { err: String(err) }));
