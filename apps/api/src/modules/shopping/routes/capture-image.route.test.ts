/**
 * Integration test for the shopping capture-image route — demo-session
 * rejection (AC6).
 *
 * DB-gated: requires DATABASE_URL, REDIS_URL, SESSION_SECRET to be set.
 * Mirrors the demo-403 pattern from capture.route.test.ts.
 *
 ...
 * before the route handler runs — parseListImage is never called, so no AI
...
function requireEnv(name: string): string {
  ...
  if (!value) {
    throw new Error(
      `capture-image.route.test.ts needs ${name} set — export it (see apps/api/.env) before running \`npm run test -w apps/api\`.`,
    );
  }
  return value;
}
...
async function buildTestApp(): Promise<FastifyInstance> {
  ...
  // Storage not needed — demo session is rejected before the route handler runs.
...
async function createTestUser(): Promise<string> {
  const [u] = await app.db
    .insert(users)
    .values({
      email: `capture-image-route-test-${randomUUID()}@example.invalid`,
      passwordHash: "x",
      displayName: "capture-image.route.test.ts user",
    })
...
test("demo session rejected on POST /api/shopping/parse-image → 403 (AC6)", async (t) => {
  ...
  // Build a minimal multipart body — won't be processed since demo is rejected first.
  ...
  const res = await app.inject({
    ...
    payload: Buffer.from(body, "latin1"),
  });
