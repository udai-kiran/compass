...
test("findInconsistentPostings: returns [] for a normally-created transaction", async (t) => {
  ...
  const acct = await createAccount(db, userId, { name: "Bank", type: "bank", institution: null, accountLast4: null, holderName: null, holderId: null, currency: "INR", openingBalancePaise: 0, schemeOpenedDate: null, nominee: "", nomineePersonId: null });
...
test("findInconsistentPostings: reports 'no postings' for a raw-inserted transaction", async (t) => {
  ...
  t.after(() => cleanupUser(userId));
  await seedSystemAccounts(db, userId);
  const _acct = await createAccount(db, userId, { name: "Bank", type: "bank", institution: null, accountLast4: null, holderName: null, holderId: null, currency: "INR", openingBalancePaise: 0, schemeOpenedDate: null, nominee: "", nomineePersonId: null });
  const [txn] = await db
    .insert(transactions)
...
test("findInconsistentPostings: tenant-scope — reports only the target user's problems", async (t) => {
  ...
  const _acctA = await createAccount(db, userA, { name: "Bank A", type: "bank", institution: null, accountLast4: null, holderName: null, holderId: null, currency: "INR", openingBalancePaise: 0, schemeOpenedDate: null, nominee: "", nomineePersonId: null });
  const _acctB = await createAccount(db, userB, { name: "Bank B", type: "bank", institution: null, accountLast4: null, holderName: null, holderId: null, currency: "INR", openingBalancePaise: 0, schemeOpenedDate: null, nominee: "", nomineePersonId: null });
